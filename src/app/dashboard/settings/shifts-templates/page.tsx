...
export default function ShiftsTemplatePage() {
	...
	return (
		<div className="px-4 sm:px-6 lg:px-8">
			...
			{/* Generate drawer */}
			<Sheet
				open={openGenerateDrawer}
				onOpenChange={(value) => !value && setOpenGenerateDrawer(false)}
			>
				<SheetContent side="right" className="sm:max-w-md overflow-y-auto">
					<SheetHeader>
		...
		try {
			if (template) {
				const response = await axios.put(
					`/api/manager/shift-templates/${template.id}`,
					data,
					{
						headers: authHeaders(),
					},
		...
		<form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
			...
			<div>
				...
				{pattern !== "custom" && blocks && (
					<p className="mt-1 text-xs text-muted-foreground">
						{t("shiftTemplates.blocks")}: {blocks.join(" – ")} (
						{blocks.reduce((a, b) => a + b, 0)}{" "}
					...
					<div className="mt-2">
						...
						<p className="mt-1 text-xs text-muted-foreground">
							...
							{blocks && blocks.length > 0 && (
								<>
									{" "}
									{t("shiftTemplates.cycle")}:{" "}
									{blocks.reduce((a, b) => a + b, 0)} {t("shiftTemplates.days")}
									.
								</>
							)}
						</p>
			...
			<div>
				<label className="block text-sm font-semibold text-foreground">
					{t("shiftTemplates.employeesLabel")}
				</label>
				<p className="mt-1 text-xs text-muted-foreground">
					{t("shiftTemplates.employeesHint")}
				</p>
					...
					<div className="mt-2 overflow-x-auto">
						{[0, 1].map((cycleIdx) => {
							...
							const cycleDays = previewData.filter(
								(r) =>
									r.day >= cycleIdx * cycleLength &&
									r.day < (cycleIdx + 1) * cycleLength,
							);
								...
								<div key={cycleIdx} className="mb-2">
									...
									<div className="flex gap-1">
										{cycleDays.map((day) => {
											const colorClass =
												EMPLOYEE_COLORS[
													day.employeeIndex % EMPLOYEE_COLORS.length
												];
											return (
												<div
													key={day.day}
			...
			<Button type="submit" variant="secondary" size="lg" className="w-full">
				{template
					? t("shiftTemplates.saveChanges")
					: t("shiftTemplates.createTemplate")}
			</Button>
		</form>
	);
}
...
function GenerateScheduleFormComponent({
	...
	const onSubmit = async (data: GenerateScheduleForm) => {
		try {
			const response = await axios.post(
				`/api/manager/shift-templates/${template.id}/generate`,
				data,
				{
					headers: authHeaders(),
				},
		...
		<form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
			<div>
				<p className="text-sm text-muted-foreground">
					{t("shiftTemplates.selectStartDate")}
				</p>
				<p className="text-sm text-muted-foreground">
					{t("shiftTemplates.selectEndDate")}
				</p>
			</div>

			<div>
				<label className="block text-sm font-semibold text-foreground">
					{t("shiftTemplates.startDate")}
				</label>
				<div className="mt-2">
					<DatePicker
						id="gen-start"
			...
			<div>
				...
				<div className="mt-2">
					<DatePicker
						...
						value={endDate}
						onChange={(date) =>
							setValue("end_date", date ? dayjs(date).format("YYYY-MM-DD") : "")
						}
